...
VBlank2:     lda PPU_STATUS
             bpl VBlank2
             ldy $fe                      ;load default cold boot pointer
...
WBootCheck:  lda TopScoreDisplay,x        ;check each score digit in the top score
             ...
             bne ColdBoot
             ldy $d6                      ;if passed both, load warm boot pointer
ColdBoot:    jsr InitializeMemory         ;clear memory using pointer in Y
             sta $4011                    ;reset delta counter load register
...
VRAM_AddrTable_Low:
      .db <VRAM_Buffer1, $FF, <GroundPaletteData
